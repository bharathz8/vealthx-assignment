...
export const detectDocumentType = (text: string): DocumentType => {
  ...
  if (
    ...
    lowerText.includes('insurance policy') || 
    ...
    lowerText.includes('vehicle insurance') || 
...
export const extractBankStatementData = (text: string): Partial<IAsset> => {
...
export const extractInsuranceData = (text: string): Partial<IAsset> => {
  const lowerText = text.toLowerCase();
  let name = '';
  let policyNumber = '';
  let insuredAmount = 0;
...
export const extractVehicleData = (text: string): Partial<IAsset> => {
